from py_dreaml_interface import dreaml_query, dreaml_learn
...
if __name__ == '__main__':
    ...
    schema = {
        ...
        'loan': [{'header': 'loan_id', 'type': 'primaryKey'}, {'header': 'loanAmt', 'type': 'continuous'},
                 {'header': 'status', 'type': 'discrete'}],
        'client': [{'header': 'cl_id', 'type': 'primaryKey'}, {'header': 'age', 'type': 'continuous'},
                   {'header': 'creditScore', 'type': 'continuous'}, {'header': 'hasAcc', 'type': 'foreignKey'}]
    }
    ...
    dreaml_learn(schema, tables, foreignKeys)
    ...
    result = dreaml_query(query, predictUncertainty, additionalQuery, additionalConditions, additionalInterventions)
    print(result)
    ...
    additionalConditions = ['status(l_137, decl)', 'savings(a_120,6300)', 'freq(a_120,low)', 'savings(a_121,6400)',
                            'freq(a_121,low)']
    additionalInterventions = ['loan(l_137)', 'account(a_120)', 'account(a_121)', 'hasLoan(a_120,l_137,true)',
                               'hasLoan(a_121,l_137,true)']
    result = dreaml_query(query, predictUncertainty, additionalQuery, additionalConditions, additionalInterventions)
    print(result)
