...
set_options(default) :-
	...
	set_debug(false),
	set_onlySample(false), %% Nitesh patch
...
%% Nitesh made this patch
set_onlySample(V) :-
	retractall(use_onlySample(_)),
	assert(use_onlySample(V)).
...
%		writeln(e(W1,DX))
	...
	%W1>0, %% Nitesh uncommented this
	W1>=0, %% Nitesh wrote this
	bb_get(dxe,DXE),
	%DXE>=DX,  %% Nitesh uncommented this
	(DXE>DX ->
		(
		check_evidence(I,PosEvidence,NegEvidence),
		bb_put(sample_sum,W1),
...
%	bb_put(nq,NQQ),
	...
	(
		...
		(
			...
			bb_get(dxq,DXEQ),
			bb_get(dx,DX2),
			%DXEQ>=DX+DX2, %% Nitesh uncommented this
			(DXEQ>DX+DX2 ->
				(
				%writeln(NW is X*W1*W2),
				NW is X*W1*W2,
	...
	ASampleList=[],
	%findall(A,recorded(I,A,_),ASampleList), %Nitesh commented this		  	
	(
	get_debug(true)->
		writeln(Query),write('Particle '),write(I),write(' w1 '),writeln(W1),write(' w2 '),writeln(W2),nl,findall(A,(recorded(I,A,_), write(A),nl),_),nl,writeln('------------------------------------');
		true
